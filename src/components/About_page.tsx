import { FunctionComponent } from "react";
import "bootstrap/dist/css/bootstrap.min.css";
import { NavBar } from "./Navigate_page";
import "../css/about.css"
import Footer from "./Footer"

interface AboutProps {}

const About: FunctionComponent<AboutProps> = () => {
  return (
    <div>
      <NavBar />
      <h1 className="text-center text-white bg-dark">About</h1>
      <div className="container">
        <div className="d-lg-flex justify-content-lg-around">
        <div className=" col-lg-4 col-sm-9 justify-content-sm-center border rounded bg-light m-3">
          <h1>bookStore</h1>
                  <p>we at book store try to help all that needs to buy good books at the low price of the current market value and we try to improve the user exprience on a daily updates of books</p> 
        </div>
        <div className="col-lg-4 d-none d-lg-block">
          <img id="image_1"
                  src="https://www.shutterstock.com/image-photo/portrait-beautiful-brunette-woman-looking-260nw-536451070.jpg"
                  alt="" 
                />
        </div>
      </div>
      <div className="d-lg-flex justify-content-lg-around">  
        <div className="col-lg-4 d-none d-lg-block">
          <img id="image_1"
                  src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTZdfp11cBsRai9qHXmmwQ_uxm_HZ3sYRUZpQ&usqp=CAU"
                  alt="" 
                />
        </div>
        <div className="col-lg-4 col-sm-9 border rounded bg-light m-3">
          <h1>a place for good times</h1>
                  <p>we try to make this a place online where you feel like you dont need to try hard for the first trying the site and we hope the user exprience would feel simplistic and understanding for you so you can relax and search away for new books for yoour collection  </p>
        </div>
      </div>

      <div className="d-lg-flex justify-content-lg-around">
          <div className="col-lg-4 col-sm-9 border rounded bg-light m-3">
          <h1>Bussiness user</h1>
                  <p>You can choose to open a bizz account for u to make a book offer on the site and also to buy books if yud like for only 5$ for the first year and 10$ for the next years of the account </p>
          </div>
          <div className="col-lg-4 d-none d-lg-block">
          <img
                  src="data:image/png;base64,"
                  alt="" 
                />
          </div>
      </div>  
    </div>
    <Footer />      
    </div>
  );
};

export default About;
